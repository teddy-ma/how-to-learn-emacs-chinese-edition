...
根据链接调到 “Bash Conditional Expressions”。如果你不想要使用鼠标，用 TAB 移到到下一个链接点然后回车也行。

和往常一样，C-h 将会显示所有的 info 模式的按键绑定。再说一次，不要去死记硬背所有的按键绑定，也不要通读单调乏味的 Info 教程。你所需要知道的就是：

* SPC 和 DEL 来滚动；当你达到了一个 『节点』（一页或者一个章节）的末尾，再次按下 SPC 就会顺序到达下一个节点。我必须警告你，DEL 有时会很淘气当你达到
一个章节的第一个节点。

* l（字母 ell）回退历史。

* u 或者 ^ 向上移动到最接近的目录。重复按下 u 会把你带到主 Info 目录包含所有的 Info 手册在你的系统中。（Emacs 相关的手册在互联网上可以能找到。）

* 使用常见的机制来搜索 (C-s and C-r)；当你达到一个节点的结尾时，再次按下 C-s 将会继续搜索剩下的手册。

正如你可以在 Info 目录下看到的，有很多 Info 手册， 包含 Emacs 自己的手册，一个 Emacs Lisp 的参考手册和介绍，和更复杂的 Emacs 编辑模式的手册，
比如 cc-mode。

Info 文档简直太好用了。如果你在维护一个 Makefile 你可以查找深奥的符号比如 $@ 的意义。需要写一些 socket 相关的程序？在后面的章节中我们将会安装
glibc (the GNU implementation of the Unix standard library) 的 Info 手册。
...
当你进入后, 使用 C-x 2 和 C-x 3 C-x o (字母 o) 来在窗口之间跳转.

无论如何, 回到 *compilation* 窗口. Compilation-mode 有它自己的特定的按键绑定, 当前:

使用 C-h m 来获得 compilation-mode 的帮助和它的按键绑定. 好消息是:  这个模式的帮助文档很短. 然后, 不要去物料的阅读所有的激活状态的副模式, 除非你确定你想看.
